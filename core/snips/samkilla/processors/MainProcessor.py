...
class MainProcessor():
	...
	SAVED_ASSISTANTS_DIR = 'var/assistants'
	...
	def initSavedIntents(self):
		for lang in listdir(self.SAVED_ASSISTANTS_DIR):
			...
			insideLangDir = "{}/{}".format(self.SAVED_ASSISTANTS_DIR, lang)
			os.makedirs(insideLangDir, exist_ok=True)
			self._savedIntents[lang] = dict()

			for projectId in listdir(insideLangDir):
				insideProjectSlotDir = "{}/{}/intents".format(insideLangDir, projectId)
				...
				for intent in listdir(insideProjectSlotDir):
					intentFilePathName = "{}/{}".format(insideProjectSlotDir, intent)
	...
	def initSavedSlots(self):
		for lang in listdir(self.SAVED_ASSISTANTS_DIR):
			...
			for projectId in listdir(insideLangDir):
				insideProjectSlotDir = "{}/{}/slots".format(insideLangDir, projectId)
				os.makedirs(insideProjectSlotDir, exist_ok=True)
				self._savedSlots[lang][projectId] = dict()

				for slot in listdir(insideProjectSlotDir):
					slotFilePathName = "{}/{}".format(insideProjectSlotDir, slot)
	...
	def initSavedAssistants(self):
		...
		for lang in listdir(self.SAVED_ASSISTANTS_DIR):
			...
			for projectId in listdir(insideLangDir):
				projectIdFilePathName = "{}/{}/_assistant.json".format(insideLangDir, projectId)
	...
	def safeBaseDicts(self, assistantId, assistantLanguage):
		...
		for baseDict in baseDicts:
			if baseDict not in self._savedAssistants[assistantLanguage][assistantId]:
				self._savedAssistants[assistantLanguage][assistantId][baseDict] = dict()

	def persistToLocalAssistantCache(self, assistantId, assistantLanguage):
		assistantMountpoint = "{}/{}/{}".format(self.SAVED_ASSISTANTS_DIR, assistantLanguage, assistantId)
		...
		with open("{}/_assistant.json".format(assistantMountpoint), 'w') as projectIdFileHandler:
	...
	def persistToGlobalAssistantSlots(self, assistantId, assistantLanguage, slotNameFilter=None):
		assistantSlotsMountpoint = "{}/{}/{}/slots".format(self.SAVED_ASSISTANTS_DIR, assistantLanguage, assistantId)
		...
		for slotTypeName in slotTypes.keys():
			...
			with open("{}/{}.json".format(assistantSlotsMountpoint, slotTypeName), 'w') as slotTypeFileHandler:
	...
	def persistToGlobalAssistantIntents(self, assistantId, assistantLanguage, intentNameFilter=None):
		assistantIntentsMountpoint = "{}/{}/{}/intents".format(self.SAVED_ASSISTANTS_DIR, assistantLanguage, assistantId)
		...
		for intentName in intents.keys():
			...
			with open("{}/{}.json".format(assistantIntentsMountpoint, intentName), 'w') as slotTypeFileHandler:
		...
		for intentName in keys:
			if intentName in self._savedIntents[intentLanguage][assistantId]:
				...
				for savedUtterance in savedIntent["utterances"]:
					if savedUtterance not in intentsGlobalValues[savedIntent["name"]]["utterances"]:
						intentsGlobalValues[savedIntent["name"]]["utterances"][savedUtterance] = True

				for moduleSlot in savedIntent["slots"]:
					if moduleSlot["name"] not in intentsGlobalValues[savedIntent["name"]]["slots"]:
						intentsGlobalValues[savedIntent["name"]]["slots"][moduleSlot["name"]] = moduleSlot
	...
	def buildMapsFromDialogTemplates(self, runOnAssistantId, moduleFilter=None, languageFilter=None):
		...
		for dir in listdir(rootDir):
			intentsPath = '{}/{}/dialogTemplate'.format(rootDir, dir)
			...
			for languageFile in listdir(intentsPath):
				moduleFile = '{}/{}'.format(intentsPath, languageFile)
				...
				for moduleIntent in module["intents"]:
					...
					if "utterances" in moduleIntent:
						for moduleUtterance in moduleIntent["utterances"]:
							if moduleUtterance not in intentsModulesValues[moduleIntent["name"]]["utterances"]:
								intentsModulesValues[moduleIntent["name"]]["utterances"][moduleUtterance] = True

					if "slots" in moduleIntent:
						for moduleSlot in moduleIntent["slots"]:
							if moduleSlot["name"] not in intentsModulesValues[moduleIntent["name"]]["slots"]:
								intentsModulesValues[moduleIntent["name"]]["slots"][moduleSlot["name"]] = moduleSlot
		...
		if len(hasDeprecatedSlotTypes) > 0:
			...
			for slotTypeName in hasDeprecatedSlotTypes:
				...
				if slotTypeName in self._savedSlots[languageFilter][runOnAssistantId]:
					...
					globalSlotTypeFile = "{}/{}/{}/slots/{}.json".format(self.SAVED_ASSISTANTS_DIR, languageFilter, runOnAssistantId, slotTypeName)
		...
		if len(hasDeprecatedIntents) > 0:
			...
			for intentName in hasDeprecatedIntents:
				...
				if intentName in self._savedIntents[languageFilter][runOnAssistantId]:
					...
					globalIntentFile = "{}/{}/{}/intents/{}.json".format(self.SAVED_ASSISTANTS_DIR, languageFilter, runOnAssistantId, intentName)
	...
	def syncRemoteToLocal(self, runOnAssistantId, moduleFilter=None, languageFilter=None):
		...
		for skill in cachedIndexedSkills:
			...
			moduleIntentsMountpoint = "{}/{}/{}".format(self.SAVED_MODULES_DIR, moduleName, "dialogTemplate")
			os.makedirs(moduleIntentsMountpoint, exist_ok=True)

			with open("{}/{}.json".format(moduleIntentsMountpoint, languageFilter), 'w') as moduleConfigFile:
