...
class DialogSessionManager(Manager):
	...
	def getUser(self, sessionId: str) -> str:
		if sessionId not in self._sessions:
			self.logWarning(f"[{self.name}] Trying to get user from a session that doesn't exist")
	...
	def addPreviousIntent(self, sessionId: str, previousIntent: str):
		if sessionId not in self._sessions:
			self.logWarning(f'[{self.name}] Was asked to add a previous intent but session was not found')
			return
