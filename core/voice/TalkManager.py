...
class TalkManager(Manager):
	...
	def getTexts(self, module, talk, strType='default') -> list:
		...
		try:
			...
			self.logWarning(f'[{self.name}] Was asked to return unexisting texts {talk} for module {module} with type {strType}')
			...
			try:
				...
				try:
					...
					try:
						...
						self.logError(f'[{self.name}] Was asked to get "{talk}" from "{module}" module in "{activeLanguage}" but it doesn\'t exist, falling back to "{defaultLanguage}" version instead')
						...
						self.logError(f'[{self.name}] Was asked to get "{talk}" from "{module}" module but language string doesn\'t exist')
						return ''
