...
class TTS(Logger):
	...
	def onStart(self):
		if self._user and self._user.ttsLanguage:
			...
			self.logInfo(f'[TTS] Language from user settings: "{self._lang}"')
		...
		if self._voice not in self._supportedLangAndVoices[self._lang][self._type]:
			...
			self.logInfo(f'[TTS] Voice "{voice}" not found for the language and type, falling back to "{self._voice}"')
		...
		if self.TTS == TTSEnum.SNIPS:
			...
			if not Path(commons.rootDir(), 'system/voices', voiceFile).exists():
				self.logInfo(f'[TTS] Using "{self.TTS.value}" as TTS with voice "{self._voice}" but voice file not found. Downloading...')
				...
				if process.returncode > 0:
					self.logError('[TTS] Failed downloading voice file, falling back to slt')
					self._voice = next(iter(self._supportedLangAndVoices[self._lang][self._type]))
