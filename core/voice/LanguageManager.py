...
class LanguageManager(Manager):
	...
	def getTranslations(self, module: str, key: str, toLang: str = '') -> Optional[list]:
		...
		if not module in self._stringsData:
			self.logError(f'[{self.name}] Asked to get translation from module "{module}" but does not exist')
			return list()
		elif key not in self._stringsData[module]:
			self.logError(f'[{self.name}] Asked to get translation for "{key}" from module "{module}" but does not exist')
			return list()
		elif toLang not in self._stringsData[module][key]:
			self.logError(f'[{self.name}] Asked to get "{toLang}" translation for "{key}" from module "{module}" but does not exist')
			return list()
		else:
	...
	def _loadSupportedLanguages(self):
		...
		if not self._activeLanguage:
			if self._defaultLanguage:
				self.logWarning(f'[{self.name}] No active language defined, falling back to {self._defaultLanguage}')
				self._activeLanguage = self._defaultLanguage
				self._activeCountryCode = self._defaultCountryCode
			else:
				self.logWarning(f'[{self.name}] No active language or default language defined, falling back to "en"')
				self._activeLanguage = 'en'
				self._activeCountryCode = 'US'
		else:
			self.logInfo(f'[{self.name}] Active language set to "{self.activeLanguageAndCountryCode}"')

		if not self._defaultLanguage:
			if self._activeLanguage:
				self.logWarning(f'[{self.name}] No default language defined, falling back to {self._activeLanguage}')
				self._defaultLanguage = self._activeLanguage
				self._defaultCountryCode = self._activeCountryCode
			else:
				self.logWarning(f'[{self.name}] No default language or active language defined, falling back to "en"')
				self._defaultLanguage = 'en'
				self._defaultCountryCode = 'US'
				self._activeLanguage = self._defaultLanguage
				self._activeCountryCode = self._defaultCountryCode
		else:
			self.logInfo(f'[{self.name}] Default language set to "{self.activeLanguageAndCountryCode}"')


		if not self._activeSnipsProjectId:
			self.logInfo(f'[{self.name}] No active snips project id set')
