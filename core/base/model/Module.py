...
class Module:
	...
	def __init__(self, supportedIntents: list, actionMapping: dict = None, authOnlyIntents: dict = None, databaseSchema: dict = None):
		...
		self._actionMapping = actionMapping
	...
	def onMessage(self, intent: str, session: DialogSession) -> bool:
		if self._actionMapping is None:
			raise NotImplementedError(f'[{self.name}] onMessage must be implemented!')

		try:
			self._actionMapping[intent](intent=intent, session=session)
			return True
		except KeyError:
			return False
