...
class Initializer:
	...
	def initProjectAlice(self) -> bool:  # NOSONAR
		...
		if not self._confsFile.exists() and not self._confsSample.exists():
			...
			self._confsFile.write_text(json.dumps({configName: configData['defaultValue'] for configName, configData in json.loads(self._confsSample.read_text()).items()}, indent=4))
