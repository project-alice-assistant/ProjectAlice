...
class Initializer:
	...
	def initProjectAlice(self) -> bool:  # NOSONAR
		...
		TEMP.write_text('deb [trusted=yes] http://debian.snips.ai/stretch stable main\ndeb [trusted=yes] http://debian.snips.ai/jessie stable main.')
		subprocess.run(['sudo', 'mv', TEMP, Path('/etc/apt/sources.list.d/snips-nlu.list')])

		subprocess.run(['sudo', 'apt', 'update'])

		subprocess.run(['sudo', 'apt', 'install', 'snips-platform-common', 'snips-nlu', 'snips-hotword', 'snips-hotword-model-heysnipsv4', 'libttspico0', 'libttspico-utils'])
		...
		try:
			...
			subprocess.run([self.PIP, 'install', 'scipy==1.3.3', 'scikit_learn==0.22.1', 'snips_nlu_utils==0.9.1', 'snips_nlu_parsers==0.4.3', 'snips_nlu==0.20.2'])
			subprocess.run(['./venv/bin/snips-nlu', 'download', confs['activeLanguage']])
